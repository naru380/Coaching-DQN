...
class PlayerOnTetris():
    ...
    def debug_mean(self, advice):
        ...
        #print("mean={}, {}".format(self.advice_q_values.eval(feed_dict={self.q_advice: [advice]}, session=self.sess), np.argmax(self.advice_q_values.eval(feed_dict={self.q_advice: [advice]}, session=self.sess))))
    ...
    def run(self, state, action, advice, mean, reward, terminal, observation):
        ...
        if self.t >= INITIAL_REPLAY_SIZE:
            ...
            if self.t % TARGET_UPDATE_INTERVAL == 0:
                self.sess.run(self.update_target_network, options=self.run_options, run_metadata=self.run_metadata)
        ...
        self.mean_net_total_q_max += np.max(self.advice_q_values.eval(feed_dict={self.q_advice: [advice]}, session=self.sess))
        self.duration += 1
    ...
    def train_network(self):
        ...
        # 勾配法による誤差最小化
        action_net_loss, _ = self.sess.run([self.loss, self.grad_update], feed_dict={
            self.q_state: np.float32(np.array(state_batch) / 255.0),
            self.action: action_batch,
            self.teacher_signal: action_net_teacher_signal_batch
            }, options=self.run_options, run_metadata=self.run_metadata)
        
        mean_net_loss, _ = self.sess.run([self.advice_net_loss, self.advice_net_grad_update], feed_dict={
