...
class Player():
    def __init__(self, num_actions, logdir_path):
        ...
        self.num_advices = num_actions # アドバイス数
            ...
            with tf.variable_scope("Player"):
                ...
                with tf.variable_scope("Action_Network"):
                    with tf.variable_scope("Q_Network"):
                        # Q Networkの構築
                        self.q_state, self.advice, self.action_q_values, action_q_network = self.build_action_network()
                        action_q_network_weights = action_q_network.trainable_weights

                    with tf.variable_scope("Target_Network"):
                        # Target Networkの構築
                        self.target_state, self.target_advice, self.action_target_q_values, action_target_network = self.build_action_network()
    ...
    def build_action_network(self):
        ...
        advice_input = Input(shape=(self.num_advices, ), dtype=tf.float32)
        ...
        #y = Dense(1, kernel_initializer='normal', name='Dense_2')(advice_input)
        ...
        q_values = model(inputs=[state, advice])

        return state, advice, q_values, model
    ...
    def get_action(self, state, advice):
        ...
        if self.t % ACTION_INTERVAL == 0:
            if self.epsilon >= random.random() or self.t < INITIAL_REPLAY_SIZE:
                ...
                action = np.argmax(self.action_q_values.eval(feed_dict={self.q_state: [np.float32(state / 255.0)], self.advice: [advice]}, session=self.sess))
    ...
    def run(self, state, action, advice, next_advice, reward, terminal, observation):
        ...
        self.action_net_total_q_max += np.max(self.action_q_values.eval(feed_dict={self.q_state: [np.float32(state / 255.0)], self.advice: [advice]}, session=self.sess))
        self.duration += 1
    ...
    def train_network(self):
       ...
       # Target Networkで次の状態でのQ値を計算
        ...
        action_net_loss, _ = self.sess.run([self.action_net_loss, self.action_net_grad_update], feed_dict={
            ...
            self.advice: np.float32(np.array(advice_batch)),
